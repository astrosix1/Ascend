...
// Date/stat helpers previously lived here as ~220 lines of free-standing
// functions inside this one screen file — moved to src/utils/dashboardStats.ts
// (same implementations, unchanged behavior) so they're reusable and testable
// on their own.
import {
  MONTH_NAMES,
  getToday,
  ...
  buildMonthlyData,
  buildYearlyData,
  buildCumulativeWeeklyData,
  buildCumulativeMonthlyData,
  buildCumulativeYearlyData,
...
export default function DashboardScreen() {
  ...
  function sanitizeHabitName(name: string): string {
    // Enforce max length of 100 characters (HTML/script stripping now shared
    // via sanitizeUserText, also used by CommunityScreen for posts/comments).
    return sanitizeUserText(name).substring(0, 100);
  }
                      ...
                      habits.map(habit => {
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            accessibilityLabel={`${habit.name}, ${isGood ? 'habit' : 'habit to avoid'}, ${isCompleted ? 'completed' : 'not completed'} on ${selectedDay}`}
                            style={[
                              styles.habitRow,
                              {
                                borderBottomColor: colors.border,
...
// Styles moved to ./DashboardScreen.styles.ts (same values, unchanged).
